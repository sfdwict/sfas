...
import org.sdfw.biometric.databinding.ActivityEarlySettlementDetailsBinding;
...
public class EarlySettlementDetailsActivity extends FingerprintComponentActivity {

    @Inject
    ViewModelProvider.Factory viewModelFactory;

    private ActivityEarlySettlementDetailsBinding mBinding;
    private EarlySettlementDetailsViewModel mViewModel;
    ...
    public void setContentView() {
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_early_settlement_details);
    }

    @Override
    public void setupViewModel(Bundle savedInstanceState) {
        mViewModel = ViewModelProviders.of(this, viewModelFactory).get(EarlySettlementDetailsViewModel.class);
        if (savedInstanceState == null) {
            ...
            mViewModel.fetchEarlySettlement(mBranchId, mCenterId, mMemberId);
        } else {
        ...
        observeFetchEarlySettlementStatus();
    ...
    private void observeFetchEarlySettlementStatus() {
        mViewModel.fetchEarlySettlementStatus().observe(this, responseWrapper -> {
            mBinding.setModel(mViewModel);
        });
    }
    ...
    private void observeValidationStatus() {
        mViewModel.getValidationStatus().observe(this, validationStatus -> {
            if (validationStatus.isSuccess()) {
                showDialog();
                mViewModel.sendEarlySettlementData();
            } else {
    ...
    private void observeDataSendStatus() {
        mViewModel.getEarlySettlementStatus().observe(this, responseWrapper -> {
            if (responseWrapper != null) {
                MessageResponse response = (MessageResponse) responseWrapper.getResponse();
                if (response.isSuccess()) {
    ...
    private void toggleFinger(boolean enable) {
        mBinding.ivFingerprintVerify.setEnabled(enable &&
                mViewModel.getEarlySettlementForm().getFields().getIsSynced().equals("N"));
    }
