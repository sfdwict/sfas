...
import org.sdfw.biometric.databinding.ActivitySavingWithdrawnDetailsBinding;
...
public class SavingWithdrawnDetailsActivity extends FingerprintComponentActivity {

    @Inject
    ViewModelProvider.Factory viewModelFactory;

    private ActivitySavingWithdrawnDetailsBinding mBinding;
    private SavingWithdrawnDetailsViewModel mViewModel;
    ...
    public void setContentView() {
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_saving_withdrawn_details);
    }

    @Override
    public void setupViewModel(Bundle savedInstanceState) {
        mViewModel = ViewModelProviders.of(this, viewModelFactory).get(SavingWithdrawnDetailsViewModel.class);
        if (savedInstanceState == null) {
            ...
            mViewModel.fetchSavingWithdrawn(mBranchId, mCenterId, mMemberId);
        } else {
        ...
        observeFetchSavingWithdrawnStatus();
    ...
    private void observeFetchSavingWithdrawnStatus() {
        mViewModel.fetchSavingWithdrawnStatus().observe(this, responseWrapper -> {
    ...
    private void observeValidationStatus() {
        mViewModel.getValidationStatus().observe(this, validationStatus -> {
            if (validationStatus.isSuccess()) {
                showDialog();
                mViewModel.sendSavingWithdrawnData();
            } else {
    ...
    private void observeDataSendStatus() {
        mViewModel.getSavingWithdrawnStatus().observe(this, responseWrapper -> {
            if (responseWrapper != null) {
                MessageResponse response = (MessageResponse) responseWrapper.getResponse();
                if (response.isSuccess()) {
    ...
    private void toggleFinger(boolean enable) {
        mBinding.ivFingerprintVerify.setEnabled(enable &&
                mViewModel.getSavingWithdrawnForm().getFields().getIsSynced().equals("N"));
    }
